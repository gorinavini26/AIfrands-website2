...
export const ResourcesView: React.FC<ResourcesViewProps> = ({
  ...
  const handleCreateSubmit = async (e: React.FormEvent) => {
    ...
    setIsCreating(true);
    try {
      await Promise.resolve(onCreateNotebook(title, newCategory, newSummary.trim(), newContent.trim()));
      setNewTitle('');
      setNewSummary('');
      setNewContent('');
            ...
            >
              ...
              <div className="flex items-center justify-between pt-4 border-t border-slate-100 text-xs font-extrabold">
                ...
                <button
                  ...
                  className="py-2 px-4 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-xs font-extrabold transition-all shadow-xs flex items-center gap-1 border-b-2 border-indigo-800 active:border-b-0 active:translate-y-0.5">
              ...
              <form onSubmit={handleCreateSubmit} className="space-y-4">
                ...
                <div className="flex gap-3 pt-2">
                  ...
                  <button
                    type="submit"
                    disabled={isCreating}
                    className="flex-1 py-3 rounded-2xl bg-amber-400 hover:bg-amber-300 disabled:opacity-60 disabled:cursor-not-allowed text-slate-950 font-extrabold text-xs border-b-4 border-amber-600 active:border-b-0 active:translate-y-0.5 shadow-md transition-all"
                  >
                    {isCreating ? 'Creating…' : 'Save Notebook'}
                  </button>
                </div>
              </form>
            </div>
